...
async def _ingest(path: str | None, *, registry_mode: bool = False, limit: int | None = None) -> None:
    settings = get_settings()
    if registry_mode:
        with SessionLocal() as session:
            registry = SourceRegistry(session)
            sources = [registry.to_config(item) for item in registry.list_sources(enabled_only=True, verified_only=True, limit=limit)]
    else:
        if not path:
...
def main() -> None:
    ...
    if args.command == "jobs":
        asyncio.run(_ingest(args.sources, registry_mode=args.registry, limit=args.limit))
        ...
        if args.source_command == "bootstrap":
            with SessionLocal() as session:
                ...
                session.commit()
            print(f"bootstrapped {len(configs)} source seeds")
