...
import { formatCountry, formatJobFamily, formatNumber } from "@/lib/utils/format";
...
export function JobsPage() {
  ...
  useEffect(() => setPage(1), [deferredQuery, country, category, sponsorship, minScore, candidateId]);
  const offset = (page - 1) * PAGE_SIZE;
  const genericParams = useMemo(() => {
    const value = new URLSearchParams({ limit: String(PAGE_SIZE), offset: String(offset) });
    ...
    return value;
  }, [category, country, deferredQuery, minScore, offset, sponsorship]);
  ...
  const generic = useJobsPage(genericParams);
