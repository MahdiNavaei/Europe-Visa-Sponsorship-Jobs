...
export function ExplanationPage() {
  ...
  useEffect(() => { if (candidateId === null) return; void api.explainRecommendations(candidateId, new URLSearchParams({ limit: "50" })).then(setData).catch(() => setError(true)); }, [candidateId]);
  if (!candidateId) return <div className="mx-auto max-w-2xl px-5 py-20 sm:px-8"><EmptyState title="Build your profile first" body="The explanation view needs a candidate profile to compare against the live job index." action="Build profile" onAction={() => router.push(`/${locale}/onboarding`)} /></div>;
